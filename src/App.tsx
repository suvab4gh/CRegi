    ...
    <div className="relative min-h-[90vh] flex flex-col items-center justify-center text-center px-6 pt-32 pb-40 overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto flex flex-col items-center">
        ...
        <Button 
          onClick={onStart}
          variant="glass"
          className="rounded-full px-14 py-8 text-lg mt-12 animate-fade-rise-delay-2 h-auto"
        >
          Launch Terminal
        </Button>
      </div>
...
export function Navbar({ currentView, setView }: { currentView: ViewState, setView: (v: ViewState) => void }) {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 px-6 py-4">
      <Card className="max-w-7xl mx-auto rounded-full px-8 py-3 flex flex-row justify-between items-center bg-white/[0.04]">
        <div 
          ...
          {navLinks.map((link) => (
            <button 
              ...
              className={cn(
                ...
                currentView === link.view ? 'text-foreground font-bold underline underline-offset-8 decoration-primary/50' : 'text-muted-foreground'
              )}
            >
              {link.label}
            </button>
        ...
        <Button 
          ...
          className="rounded-full px-8 py-2 h-10 text-[10px] uppercase tracking-widest transition-all"
        >
          Launch
        </Button>
      </Card>
